...
import {
  PERMISO_GENERAL_CHECKLIST,
  PERMISO_GENERAL_REQUISITOS_PERSONAS,
...
export default function PermisoGeneralPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!complementario) throw new Error('Este expediente no tiene un Permiso de Trabajo General requerido.')
        ...
        for (const item of PERMISO_GENERAL_CHECKLIST) {
        ...
        if (active) setError(err instanceof Error ? err.message : 'No se pudo cargar el Permiso de Trabajo General.')
      } finally {
  ...
  const pendientes = useMemo(
    () => PERMISO_GENERAL_CHECKLIST.filter((item) => !respuestas[item.codigo]?.respuesta).length,
    [respuestas]
  )

  const bloqueantesNo = useMemo(
    () =>
      PERMISO_GENERAL_CHECKLIST.filter(
        (item) => item.bloqueanteSiNo && respuestas[item.codigo]?.respuesta === 'no'
      ).length,
    [respuestas]
  )

  const completos = PERMISO_GENERAL_CHECKLIST.length - pendientes
  const porcentaje = Math.round((completos / PERMISO_GENERAL_CHECKLIST.length) * 100)
  ...
  const guardar = async () => {
    ...
    try {
      ...
      const rows = PERMISO_GENERAL_CHECKLIST.map((item) => ({
      ...
      const { error: historialError } = await supabase.from('pts_historial').insert({
        ...
        empresa_id: empresaId,
        evento: 'permiso_general_guardado',
        detalle: listo
          ? 'Permiso de Trabajo General completado sin respuestas bloqueantes.'
          : bloqueantesNo > 0
            ? `Permiso de Trabajo General guardado con ${bloqueantesNo} respuesta(s) NO bloqueante(s).`
            : `Permiso de Trabajo General guardado con ${pendientes} respuesta(s) pendiente(s).`,
      })
      ...
      setSuccess(
        listo
          ? 'Permiso de Trabajo General completo y habilitado para continuar el flujo.'
          : bloqueantesNo > 0
      ...
      setError(err instanceof Error ? err.message : 'No se pudo guardar el Permiso de Trabajo General.')
    } finally {
      setSaving(false)
    }
  }

  const secciones = Array.from(new Set(PERMISO_GENERAL_CHECKLIST.map((item) => item.seccion)))
    ...
    <PTSAccessGuard>
      <main className="mx-auto max-w-6xl space-y-6 px-6 py-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <Link href={`/seguridad/pts/${permisoId}`} className="text-sm font-medium text-slate-500 hover:text-slate-900">← Volver al expediente</Link>
            <p className="mt-4 text-xs font-semibold uppercase tracking-[0.18em] text-[#168F86]">Permiso complementario</p>
            <h1 className="mt-1 text-3xl font-semibold text-slate-900">Permiso de Trabajo General</h1>
            <p className="mt-2 text-sm text-slate-500">RE-MO9-PR-04 · Responder todos los controles antes de solicitar revisión.</p>
          </div>
          {permiso ? (
          ...
          <>
            <section className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">{completos} de {PERMISO_GENERAL_CHECKLIST.length} controles respondidos</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                {PERMISO_GENERAL_REQUISITOS_PERSONAS.map((requisito) => (
            ...
            {secciones.map((seccion) => (
              <section key={seccion} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">Selecciona Sí, No o N/A. Una respuesta No en un control bloqueante impide enviar el expediente a revisión.</p>
                <div className="mt-5 space-y-4">
                  {PERMISO_GENERAL_CHECKLIST.filter((item) => item.seccion === seccion).map((item) => {
                    ...
                    return (
                      <div key={item.codigo} className={`rounded-2xl border p-4 ${bloqueado ? 'border-red-200 bg-red-50/50' : 'border-slate-200 bg-white'}`}>
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="max-w-2xl">
                            <div className="flex items-center gap-2">
                              ...
                              {item.bloqueanteSiNo ? <span className="text-[11px] font-semibold uppercase tracking-wide text-red-500">Bloqueante</span> : null}
                            </div>
                        ...
                        <textarea
                          ...
                          rows={2}
                          placeholder={bloqueado ? 'Indica la condición que impide aprobar este punto y la corrección requerida...' : 'Observación opcional...'}
            ...
            {bloqueantesNo > 0 ? (
              <section className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">
                <strong>Permiso bloqueado:</strong> existe al menos una respuesta NO. Debe corregirse la condición y actualizar la respuesta antes de continuar a revisión.
              </section>
            ) : null}

            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              <Link href={`/seguridad/pts/${permisoId}`} className="inline-flex justify-center rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-medium text-slate-700">Volver al expediente</Link>
              <button onClick={guardar} disabled={saving} className="inline-flex justify-center rounded-xl bg-[#18B7A8] px-5 py-3 text-sm font-semibold text-white hover:bg-[#11998E] disabled:opacity-60">
                {saving ? 'Guardando...' : listo ? 'Guardar permiso completo' : 'Guardar borrador'}
              </button>
            </div>
          </>
        ) : null}
