...
import {
  PERMISO_EXCAVACION_CHECKLIST,
  PERMISO_EXCAVACION_REQUISITOS_PERSONAS,
} from '../../../../../../lib/pts/excavacion-checklist'
...
export default function PermisoExcavacionPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!complementario) throw new Error('Este expediente no tiene un Permiso de Excavación requerido.')
        ...
        for (const item of PERMISO_EXCAVACION_CHECKLIST) {
        ...
        if (active) setError(err instanceof Error ? err.message : 'No se pudo cargar el Permiso de Excavación.')
      } finally {
  ...
  const pendientes = useMemo(
    () => PERMISO_EXCAVACION_CHECKLIST.filter((item) => !respuestas[item.codigo]?.respuesta).length,
    [respuestas]
  )

  const bloqueantesNo = useMemo(
    () => PERMISO_EXCAVACION_CHECKLIST.filter(
      (item) => item.bloqueanteSiNo && respuestas[item.codigo]?.respuesta === 'no'
    ).length,
    [respuestas]
  )

  const naInvalidos = useMemo(
    () => PERMISO_EXCAVACION_CHECKLIST.filter(
      (item) => respuestas[item.codigo]?.respuesta === 'na' && !item.permiteNA
    ).length,
    [respuestas]
  )

  const naSinJustificacion = useMemo(
    () => PERMISO_EXCAVACION_CHECKLIST.filter(
      (item) =>
        item.permiteNA &&
        respuestas[item.codigo]?.respuesta === 'na' &&
        !respuestas[item.codigo]?.observacion.trim()
    ).length,
    [respuestas]
  )

  const completos = PERMISO_EXCAVACION_CHECKLIST.length - pendientes
  const porcentaje = Math.round((completos / PERMISO_EXCAVACION_CHECKLIST.length) * 100)
  const listo = pendientes === 0 && bloqueantesNo === 0 && naInvalidos === 0 && naSinJustificacion === 0

  const setRespuesta = (codigo: string, respuesta: Respuesta) => {
    const definicion = PERMISO_EXCAVACION_CHECKLIST.find((item) => item.codigo === codigo)
  ...
  const guardar = async () => {
    ...
    try {
      ...
      const { error: historialError } = await supabase.from('pts_historial').insert({
        ...
        empresa_id: empresaId,
        evento: 'permiso_excavacion_guardado',
        detalle: listo
  ...
  const secciones = Array.from(new Set(PERMISO_EXCAVACION_CHECKLIST.map((item) => item.seccion)))
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-6 py-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold text-slate-900">Permiso de Excavación</h1>
            <p className="mt-2 text-sm text-slate-500">RE-MO9-PR-01 · Una respuesta NO bloquea la autorización y la ejecución del trabajo.</p>
          </div>
          {permiso ? (
          ...
          <>
            <section className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">{completos} de {PERMISO_EXCAVACION_CHECKLIST.length} controles respondidos</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className={`rounded-2xl border p-5 shadow-sm ${bloqueantesNo > 0 ? 'border-red-200 bg-red-50' : 'border-emerald-200 bg-emerald-50'}`}>
                ...
                <p className={`mt-1 text-sm ${bloqueantesNo > 0 ? 'text-red-600' : 'text-emerald-700'}`}>{bloqueantesNo > 0 ? 'La excavación no puede autorizarse' : 'Sin bloqueos registrados'}</p>
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                {PERMISO_EXCAVACION_REQUISITOS_PERSONAS.map((requisito) => (
            ...
            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">
              <strong>Criterio de hiperseguridad:</strong> los controles esenciales de AST, delimitación, accesos, rescate, servicios enterrados, control de derrumbe, vigía y disposición del material excavado deben responderse Sí o No. N/A solo se permite cuando el control depende realmente del equipo o de una protección contra caídas no aplicable, y debe justificarse.
            </section>
            ...
            {secciones.map((seccion) => (
              <section key={seccion} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">Cada respuesta NO bloquea el permiso hasta corregir la condición y reevaluar. Todo N/A permitido exige una justificación técnica.</p>
                <div className="mt-5 space-y-4">
                  {PERMISO_EXCAVACION_CHECKLIST.filter((item) => item.seccion === seccion).map((item) => {
                    ...
                    return (
                      <div key={item.codigo} className={`rounded-2xl border p-4 ${bloqueado || naSinDetalle ? 'border-red-200 bg-red-50/50' : 'border-slate-200 bg-white'}`}>
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="max-w-2xl">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {!item.permiteNA ? <span className="text-[11px] font-semibold uppercase tracking-wide text-amber-600">N/A no permitido</span> : <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">N/A requiere justificación</span>}
                            </div>
                        ...
                        <textarea
                          ...
                          placeholder={
                            bloqueado
                              ? 'Indica la condición insegura y la corrección requerida antes de reevaluar...'
                              : row.respuesta === 'na'
                                ? 'Justifica técnicamente por qué este control no aplica...'
            ...
            {bloqueantesNo > 0 ? (
              <section className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">
                <strong>Permiso bloqueado:</strong> existe al menos una respuesta NO. La excavación no puede ser autorizada hasta corregir la condición y reevaluar el punto.
              </section>
            ) : null}
