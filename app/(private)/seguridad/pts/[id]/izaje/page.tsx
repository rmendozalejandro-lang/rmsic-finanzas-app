...
import {
  PERMISO_IZAJE_CHECKLIST,
  PERMISO_IZAJE_REQUISITOS_PERSONAS,
...
export default function PermisoIzajePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!complementario) throw new Error('Este expediente no tiene un Permiso de Maniobras de Izaje requerido.')
        ...
        for (const item of PERMISO_IZAJE_CHECKLIST) {
        ...
        if (active) setError(err instanceof Error ? err.message : 'No se pudo cargar el Permiso de Maniobras de Izaje.')
      } finally {
  ...
  const pendientes = useMemo(
    () => PERMISO_IZAJE_CHECKLIST.filter((item) => !respuestas[item.codigo]?.respuesta).length,
    [respuestas]
  )

  const bloqueantesNo = useMemo(
    () =>
      PERMISO_IZAJE_CHECKLIST.filter(
        (item) => item.bloqueanteSiNo && respuestas[item.codigo]?.respuesta === 'no'
      ).length,
    [respuestas]
  )

  const respuestasNaInvalidas = useMemo(
    () =>
      PERMISO_IZAJE_CHECKLIST.filter(
        (item) => !item.permiteNA && respuestas[item.codigo]?.respuesta === 'na'
      ).length,
    [respuestas]
  )

  const completos = PERMISO_IZAJE_CHECKLIST.length - pendientes
  const porcentaje = Math.round((completos / PERMISO_IZAJE_CHECKLIST.length) * 100)
  const listo = pendientes === 0 && bloqueantesNo === 0 && respuestasNaInvalidas === 0

  const setRespuesta = (codigo: string, respuesta: Respuesta) => {
    const definicion = PERMISO_IZAJE_CHECKLIST.find((item) => item.codigo === codigo)
    if (respuesta === 'na' && definicion && !definicion.permiteNA) return
  ...
  const guardar = async () => {
    ...
    try {
      ...
      const rows = PERMISO_IZAJE_CHECKLIST.map((item) => ({
      ...
      const nuevoEstado = listo ? 'completo' : bloqueantesNo > 0 || respuestasNaInvalidas > 0 ? 'observado' : 'borrador'
      ...
      const { error: historialError } = await supabase.from('pts_historial').insert({
        ...
        empresa_id: empresaId,
        evento: 'permiso_izaje_guardado',
        detalle: listo
          ? 'Permiso de Maniobras de Izaje completado sin respuestas bloqueantes.'
          : bloqueantesNo > 0
            ? `Permiso de Maniobras de Izaje guardado con ${bloqueantesNo} respuesta(s) NO bloqueante(s).`
            : respuestasNaInvalidas > 0
              ? `Permiso de Maniobras de Izaje guardado con ${respuestasNaInvalidas} respuesta(s) N/A no admitida(s).`
              : `Permiso de Maniobras de Izaje guardado con ${pendientes} respuesta(s) pendiente(s).`,
      })
      ...
      setSuccess(
        listo
          ? 'Permiso de Maniobras de Izaje completo y habilitado para continuar el flujo.'
          : bloqueantesNo > 0
            ? 'Guardado. La maniobra permanece bloqueada porque existe al menos una respuesta NO.'
            : respuestasNaInvalidas > 0
              ? 'Guardado. Existen respuestas N/A en controles que deben verificarse obligatoriamente.'
              : 'Borrador guardado. Aún quedan controles pendientes.'
      )
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo guardar el Permiso de Maniobras de Izaje.')
    } finally {
      setSaving(false)
    }
  }

  const secciones = Array.from(new Set(PERMISO_IZAJE_CHECKLIST.map((item) => item.seccion)))
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-6 py-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold text-slate-900">Permiso de Maniobras de Izaje</h1>
            <p className="mt-2 text-sm text-slate-500">RE-MO9-PR-05 · Una respuesta NO bloquea la autorización y la ejecución de la maniobra.</p>
          </div>
          {permiso ? (
          ...
          <>
            <section className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">{completos} de {PERMISO_IZAJE_CHECKLIST.length} controles respondidos</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className={`rounded-2xl border p-5 shadow-sm ${bloqueantesNo > 0 ? 'border-red-200 bg-red-50' : 'border-emerald-200 bg-emerald-50'}`}>
                ...
                <p className={`mt-1 text-sm ${bloqueantesNo > 0 ? 'text-red-600' : 'text-emerald-700'}`}>{bloqueantesNo > 0 ? 'La maniobra no puede autorizarse' : 'Sin bloqueos registrados'}</p>
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                {PERMISO_IZAJE_REQUISITOS_PERSONAS.map((requisito) => (
            ...
            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">
              <strong>Criterio de hiperseguridad:</strong> N/A solo está disponible cuando la configuración del equipo puede hacer que el control no aplique. Los controles esenciales de AST, personal autorizado, segregación, capacidad de carga, inspección y líneas energizadas deben responderse Sí o No.
            </section>
            ...
            {secciones.map((seccion) => (
              <section key={seccion} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">Cada respuesta NO bloquea el permiso hasta corregir la condición y reevaluar. N/A se habilita solo en controles técnicamente condicionados al equipo utilizado.</p>
                <div className="mt-5 space-y-4">
                  {PERMISO_IZAJE_CHECKLIST.filter((item) => item.seccion === seccion).map((item) => {
                    ...
                    return (
                      <div key={item.codigo} className={`rounded-2xl border p-4 ${bloqueado ? 'border-red-200 bg-red-50/50' : 'border-slate-200 bg-white'}`}>
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="max-w-2xl">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-[11px] font-semibold uppercase tracking-wide text-red-500">Bloqueante si es NO</span>
                              {!item.permiteNA ? <span className="text-[11px] font-semibold uppercase tracking-wide text-amber-600">N/A no permitido</span> : null}
                            </div>
                            <p className="mt-2 text-sm font-medium text-slate-900">{item.pregunta}</p>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {(['si', 'no', 'na'] as const).map((value) => {
                              if (value === 'na' && !item.permiteNA) return null
                        ...
                        <textarea
                          ...
                          rows={2}
                          placeholder={bloqueado ? 'Indica la condición insegura, acción correctiva y condición requerida antes de reevaluar...' : row.respuesta === 'na' ? 'Justifica por qué este control no aplica al equipo o maniobra...' : 'Observación opcional...'}
            ...
            {bloqueantesNo > 0 ? (
              <section className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">
                <strong>Permiso bloqueado:</strong> existe al menos una respuesta NO. La maniobra de izaje no puede ser autorizada ni ejecutada hasta corregir la condición y reevaluar el control.
              </section>
            ) : null}
