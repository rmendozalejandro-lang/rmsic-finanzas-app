...
function getPreviewDateRange(filas: FilaPreview[]) {
  const validDates = filas
    .map((fila) => fila.fecha)
    .filter((fecha): fecha is string => Boolean(fecha))
    .sort();

  return {
    desde: validDates[0] ?? null,
    hasta: validDates[validDates.length - 1] ?? null,
  };
}
...
function BancosImportarContent() {
  ...
  const [ultimaImportacion, setUltimaImportacion] =
    useState<UltimaImportacionCuenta | null>(null);
  const [loadingCuentas, setLoadingCuentas] = useState(true);
  const [loadingUltimaImportacion, setLoadingUltimaImportacion] =
    useState(false);
  ...
  useEffect(() => {
    const loadUltimaImportacion = async () => {
      try {
        setUltimaImportacion(null);

        if (!empresaId || !cuentaBancariaId) return;

        setLoadingUltimaImportacion(true);

        const { data, error: ultimaError } = await supabase
          .from("v_banco_ultima_importacion_por_cuenta")
          .select(
            "importacion_id, banco, nombre_cuenta, numero_cuenta, nombre_archivo, fecha_desde, fecha_hasta, fecha_maxima_real, total_filas, total_duplicadas, filas_pendientes, filas_conciliadas, estado, created_at",
          )
          .eq("empresa_id", empresaId)
        ...
        if (ultimaError) {
          throw new Error(ultimaError.message);
        }

        setUltimaImportacion((data ?? null) as UltimaImportacionCuenta | null);
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "No se pudo cargar la última importación de esta cuenta.",
        );
      } finally {
        setLoadingUltimaImportacion(false);
      }
    };

    void loadUltimaImportacion();
  }, [empresaId, cuentaBancariaId]);
  ...
  const ultimaFechaCargada = useMemo(() => {
    return (
      ultimaImportacion?.fecha_hasta ??
      ultimaImportacion?.fecha_maxima_real ??
      null
    );
  }, [ultimaImportacion]);

  const proximaFechaSugerida = useMemo(() => {
    return addDaysToDateString(ultimaFechaCargada, 1);
  }, [ultimaFechaCargada]);

  const previewDateRange = useMemo(() => {
    return getPreviewDateRange(preview?.filas ?? []);
  }, [preview]);

  const previewSobreponeUltimaCarga = useMemo(() => {
    const comparison = compareDateStrings(
      previewDateRange.desde,
      ultimaFechaCargada,
    );
    return comparison != null ? comparison <= 0 : false;
  }, [previewDateRange.desde, ultimaFechaCargada]);
  ...
  const handleConfirmarImportacion = async () => {
    try {
      ...
      if (!preview) {
        throw new Error(
          "Primero debes leer una cartola y revisar la vista previa.",
        );
      }
  ...
  const handlePreview = async () => {
    try {
      ...
      setSuccess(
        "Cartola leída correctamente. Revisa la vista previa antes de importar.",
      );
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "No se pudo procesar la cartola.",
      );
    } finally {
      setProcesando(false);
    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Sube una cartola Excel de BancoEstado o BCI para generar una vista
              previa normalizada antes de crear movimientos.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <div>
            ...
            <select
              ...
              onChange={(event) => {
                setCuentaBancariaId(event.target.value);
                setPreview(null);
                setImportacionId("");
                setSuccess("");
                setError("");
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="font-semibold">
                  Última importación de esta cuenta
                </p>

                {loadingUltimaImportacion ? (
                  ...
                  <div className="mt-2 space-y-2">
                    <p>
                      Último rango cargado:{" "}
                      <span className="font-semibold">
                        {formatDate(ultimaImportacion.fecha_desde)} al{" "}
                        {formatDate(ultimaImportacion.fecha_hasta)}
                      </span>
                    </p>

                    <p className="break-all text-blue-800">
                      Archivo:{" "}
                    ...
                    <div className="grid gap-2 pt-1 sm:grid-cols-2 lg:grid-cols-4">
                      <div className="rounded-xl bg-white/70 px-3 py-2">
                        <p className="text-xs text-blue-700">Movimientos</p>
                        <p className="font-semibold">
                          {ultimaImportacion.total_filas ?? 0}
                        </p>
                      </div>

                      <div className="rounded-xl bg-white/70 px-3 py-2">
                        <p className="text-xs text-blue-700">Duplicados</p>
                        <p className="font-semibold">
                          {ultimaImportacion.total_duplicadas ?? 0}
                        </p>
                      </div>
                      ...
                      <div className="rounded-xl bg-white/70 px-3 py-2">
                        ...
                        <p className="font-semibold">
                          {ultimaImportacion.filas_pendientes ?? 0}
                        </p>
                      </div>

                      <div className="rounded-xl bg-white/70 px-3 py-2">
                        <p className="text-xs text-blue-700">Conciliadas</p>
                        <p className="font-semibold">
                          {ultimaImportacion.filas_conciliadas ?? 0}
                        </p>
                      </div>
                    </div>

                    <p className="text-xs text-blue-700">
                      {ultimaFechaCargada && proximaFechaSugerida
                        ? `Recomendación: esta cuenta ya tiene movimientos cargados hasta el ${formatDate(ultimaFechaCargada)}. Descarga la próxima cartola desde el ${formatDate(proximaFechaSugerida)}.`
                        : "Recomendación: revisa el historial antes de cargar una nueva cartola para evitar repetir movimientos."}
                    </p>
                  </div>
                ) : (
                  <p className="mt-1 text-blue-700">
                    No hay importaciones registradas para esta cuenta. Puedes
                    cargar la primera cartola bancaria.
                  </p>
                )}
              </div>
              ...
              <Link
                ...
                className="inline-flex items-center justify-center rounded-xl border border-blue-300 bg-white px-4 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-100"
              >
                Ver historial
              </Link>
            </div>
      ...
      {preview ? (
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {previewSobreponeUltimaCarga ? (
              <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
                <p className="font-semibold">Posible rango ya cargado</p>
                <p className="mt-1">
                  La cartola leída contiene movimientos desde{" "}
                  <span className="font-semibold">
                    {formatDate(previewDateRange.desde)}
                  </span>{" "}
                  hasta{" "}
                  <span className="font-semibold">
                    {formatDate(previewDateRange.hasta)}
                  </span>
                  , y esta cuenta ya tiene movimientos cargados hasta{" "}
                  <span className="font-semibold">
                    {formatDate(ultimaFechaCargada)}
                  </span>
                  .
                </p>
                <p className="mt-1 text-xs">
                  ...
                  <span className="font-semibold">
                    {proximaFechaSugerida
                      ? formatDate(proximaFechaSugerida)
                      : "la fecha siguiente"}
                  </span>
                  .
                </p>
            ...
            <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1">
                  Se guardarán las filas válidas en bandeja de revisión. Los
                  duplicados serán omitidos.
                </p>
                {importacionId ? (
