      ...
      <div className="space-y-4">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Vuelve al dashboard, selecciona una empresa y entra nuevamente a este módulo.
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-semibold text-slate-900">{resumen.borrador}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-slate-500">Enviadas</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{resumen.enviada}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-slate-500">Aprobadas</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{resumen.aprobada}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-slate-500">Rechazadas</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{resumen.rechazada}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {loading ? (
          <div className="p-6 text-sm text-slate-600">Cargando cotizaciones...</div>
        ) : error ? (
          <div className="p-5">
          ...
          <div className="p-8 text-center">
            <p className="text-sm text-slate-600">No hay cotizaciones para mostrar.</p>
            <div className="mt-4">
              <Link
                href="/cotizaciones/nueva"
                className="inline-flex items-center rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-800"
                ...
                {filteredCotizaciones.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-slate-50/70">
                      ...
                      <td className="px-5 py-4">
                        <div className="text-slate-700">{formatDate(row.fecha_vencimiento)}</div>
                        {expired && row.estado !== "vencida" ? (
                          <div className="text-xs font-medium text-amber-700">Fecha vencida</div>
                        ) : null}
                      </td>
                      <td className="px-5 py-4">
                        <span
