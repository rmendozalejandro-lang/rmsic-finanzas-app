...
export default function AsientosContablesPage() {
  ...
  const handleContabilizar = async (asiento: AsientoResumen) => {
    if (!canManage) {
      setError('Solo usuarios admin pueden contabilizar asientos.')
      return
    }
    ...
    if (!asiento.cuadrado || Number(asiento.diferencia || 0) !== 0 || asiento.lineas < 2) {
      setError('El asiento debe estar cuadrado y tener al menos 2 líneas.')
      return
    }

    const confirmed = window.confirm(
      `¿Deseas contabilizar el asiento ${asiento.numero || 'sin número'}? Esta acción cambiará su estado de borrador a contabilizado.`
    )
    ...
    try {
      ...
      await loadAsientos(empresaActivaId)
      await loadDetalle(asiento.id)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo contabilizar el asiento.')
    } finally {
      setSaving(false)
    }
  }
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          {canManage && asiento.estado === 'borrador' && asiento.cuadrado && (
                            <button
                              type="button"
                              onClick={() => void handleContabilizar(asiento)}
                              disabled={saving}
                              className="rounded-xl border border-emerald-300 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60"
                            >
                              Contabilizar
                            </button>
                          )}
