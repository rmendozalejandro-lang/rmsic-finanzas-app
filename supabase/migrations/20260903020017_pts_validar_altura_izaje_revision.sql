...
begin
  ...
  if v_general_requerido > 0 then
    select
      count(*) filter (where c.codigo_item = any (array[
        ...
        'GEN-CHK-01','GEN-CHK-02','GEN-CHK-03','GEN-CHK-04','GEN-CHK-05','GEN-CHK-06','GEN-CHK-07','GEN-CHK-08','GEN-CHK-09','GEN-CHK-10','GEN-CHK-11','GEN-CHK-12','GEN-CHK-13','GEN-CHK-14','GEN-CHK-15'
      ]::text[])),
      count(*) filter (where c.codigo_item = any (array[
        'GEN-EPP-01','GEN-EPP-02','GEN-EPP-03','GEN-EPP-04','GEN-EPP-05','GEN-EPP-06','GEN-EPP-07','GEN-EPP-08','GEN-EPP-09',
        'GEN-CHK-01','GEN-CHK-02','GEN-CHK-03','GEN-CHK-04','GEN-CHK-05','GEN-CHK-06','GEN-CHK-07','GEN-CHK-08','GEN-CHK-09','GEN-CHK-10','GEN-CHK-11','GEN-CHK-12','GEN-CHK-13','GEN-CHK-14','GEN-CHK-15'
      ]::text[]) and c.respuesta is null),
      count(*) filter (where c.codigo_item = any (array[
        'GEN-EPP-01','GEN-EPP-02','GEN-EPP-03','GEN-EPP-04','GEN-EPP-05','GEN-EPP-06','GEN-EPP-07','GEN-EPP-08','GEN-EPP-09',
        'GEN-CHK-01','GEN-CHK-02','GEN-CHK-03','GEN-CHK-04','GEN-CHK-05','GEN-CHK-06','GEN-CHK-07','GEN-CHK-08','GEN-CHK-09','GEN-CHK-10','GEN-CHK-11','GEN-CHK-12','GEN-CHK-13','GEN-CHK-14','GEN-CHK-15'
      ]::text[]) and c.respuesta = 'no')
    ...
    if v_general_respuestas <> 24 then
      raise exception 'El Permiso de Trabajo General debe tener sus 24 controles esperados registrados antes de revision';
    end if;
  ...
  select count(*) into v_altura_requerido
  from public.pts_permisos_complementarios
  where permiso_id = p_permiso_id
    and empresa_id = v_empresa_id
    and tipo = 'altura'
  ...
  if v_altura_requerido > 0 then
    select
      count(*) filter (where c.codigo_item = any (array[
        ...
        'ALT-EPP-01','ALT-EPP-02','ALT-EPP-03','ALT-EPP-04','ALT-EPP-05','ALT-EPP-06','ALT-EPP-07','ALT-EPP-08'
      ]::text[])),
      count(*) filter (where c.codigo_item = any (array[
        'ALT-CHK-01','ALT-CHK-02','ALT-CHK-03','ALT-CHK-04','ALT-CHK-05','ALT-CHK-06','ALT-CHK-07','ALT-CHK-08','ALT-CHK-09','ALT-CHK-10','ALT-CHK-11','ALT-CHK-12','ALT-CHK-13','ALT-CHK-14',
        'ALT-EPP-01','ALT-EPP-02','ALT-EPP-03','ALT-EPP-04','ALT-EPP-05','ALT-EPP-06','ALT-EPP-07','ALT-EPP-08'
    ...
    if v_altura_respuestas <> 22 then
      raise exception 'El Permiso de Trabajo en Altura debe tener sus 22 controles esperados registrados antes de revision';
    end if;
    if v_altura_pendientes > 0 then
      raise exception 'El Permiso de Trabajo en Altura tiene respuestas pendientes';
    end if;
    if v_altura_no > 0 then
      raise exception 'El Permiso de Trabajo en Altura contiene respuestas NO y no puede aprobarse';
    end if;
  end if;

  select count(*) into v_izaje_requerido
  from public.pts_permisos_complementarios
  where permiso_id = p_permiso_id
    and empresa_id = v_empresa_id
    and tipo = 'izaje'
  ...
  if v_izaje_requerido > 0 then
    select
      count(*) filter (where c.codigo_item = any (array[
        'IZA-CHK-01','IZA-CHK-02','IZA-CHK-03','IZA-CHK-04','IZA-CHK-05','IZA-CHK-06','IZA-CHK-07','IZA-CHK-08','IZA-CHK-09','IZA-CHK-10','IZA-CHK-11','IZA-CHK-12','IZA-CHK-13'
      ]::text[])),
      count(*) filter (where c.codigo_item = any (array[
        'IZA-CHK-01','IZA-CHK-02','IZA-CHK-03','IZA-CHK-04','IZA-CHK-05','IZA-CHK-06','IZA-CHK-07','IZA-CHK-08','IZA-CHK-09','IZA-CHK-10','IZA-CHK-11','IZA-CHK-12','IZA-CHK-13'
      ]::text[]) and c.respuesta is null),
    ...
    if v_izaje_respuestas <> 13 then
      raise exception 'El Permiso de Maniobras de Izaje debe tener sus 13 controles esperados registrados antes de revision';
    end if;
    if v_izaje_pendientes > 0 then
      raise exception 'El Permiso de Maniobras de Izaje tiene respuestas pendientes';
    end if;
    if v_izaje_no > 0 then
      raise exception 'El Permiso de Maniobras de Izaje contiene respuestas NO y no puede aprobarse';
    end if;
  end if;
